...
object TestRunner extends App
{
  ...
  def compileAsmToBin(asmFileName: String): Option[String] = {
    ...
    if (virtualMode)
    {
      ...
      process = "riscv32-unknown-elf-gcc -static -mcmodel=medany -fvisibility=hidden -nostdlib -nostartfiles -Wa,-march=rv64im -DENTROPY=" + entropy + " -std=gnu99 -O2 -I./env/v -I./macros/scalar -T./env/v/link.ld ./env/v/entry.S ./env/v/vm.c " + asmFileName + " -lc -o " + binFileName
    }
    else
    {
      println("Physical mode")
      process = "riscv32-unknown-elf-gcc -nostdlib -nostartfiles -Wa,-march=rv32im -I./env/p -T./env/p/link.ld " + asmFileName + " -o " + binFileName
    }
